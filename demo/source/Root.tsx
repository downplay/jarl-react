...
// Route location shape is intentionally dynamic here (mirrors jarl-react's own
// `Location = Record<string, any>` design), so match/state callback params below
// are `any`.
const routes = [
    {
        path: "/",
...
class Root extends Component<{}, RootState> {
    ...
    // eslint-disable-next-line @typescript-eslint/no-explicit-any
    handleChange = ({ location }: any): void => {
        this.setState({
            // Update
            location
        });
    ...
    // eslint-disable-next-line @typescript-eslint/no-explicit-any
    handleDemoError = (error: any, info: any): void => {
