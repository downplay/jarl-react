// This page renders API reference data produced at build time by the
// `react-docgen`/`documentation.js` webpack loaders (see ../docs/api). That
// output's shape is dynamic/tool-generated, so it and everything derived from it
// below is typed `any` throughout this file - a deliberate dynamic-boundary choice
...
const toApi = (apiName: string) => ({ page: "api", apiName });
...
const apiTitle = (apiName: string) =>
    apis.find(api => api.name === apiName)!.title;

const PreCell = ({ children, ...rest }: any) => (
    <Table.Cell {...rest}>
        <pre>{children}</pre>
    </Table.Cell>
);
...
const flattenParas = (root: any): string =>
...
const PropsHeaderRow = ({ showRequired }: any) => (
    <Table.Header>
        <Table.Row>
...
const translateType = (type: any): string => {
    if (!type) {
        return "";
    }
    switch (type.type) {
...
const ClassApi = ({ item }: any) => (
    <Fragment>
        ...
        {item.members.instance.map((member: any) => (
            <Member
                item={item}
                member={member}
...
const Api = ({ apiName }: ApiProps) => (
    <Page>
        ...
        <Body>
            {(apiContent as any)[apiName].map((item: any) => (
