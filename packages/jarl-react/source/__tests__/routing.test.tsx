...
describe("location", () => {
    test("passes arbitrary location props to component", () => {
        ...
        const props: any = test.props();
        expect(props.foo).toEqual(mockState.foo);
    });

    test("maps specific props", () => {
        const TestComponent = routing<any>(({ foo }: any) => ({ baz: foo }))(
            mockComponent
        );
        const mockState = { foo: Symbol("bar") };
        const output = mount(
...
describe("isActive", () => {
    ...
    beforeEach(() => {
        ...
        TestComponent = routing(null, ({ isActive }: any, { to }: any) => ({
            active: isActive(to)
        }))(mockComponent);
    });
...
describe("resolved", () => {
    ...
    test("passes mapped resolve props to component", async () => {
        const TestComponent = routing<any>(
            null,
            null,
            ({ Foo }: any) => ({ Bar: Foo })
        )(mockComponent);
