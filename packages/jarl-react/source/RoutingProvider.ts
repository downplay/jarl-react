...
import { History, LocationState as HistoryLocationState } from "history";

import RouteMap, { joinPaths, hydrateRoute } from "./RouteMap";
import safeJsonStringify from "./lib/safeJsonStringify";
import { Redirect } from "./redirect";
import { Location, MappedRoute, RouteDefinition, RoutingContext } from "./types";
...
const ensureRouteMap = (routes: RouteDefinition[] | RouteMap): RouteMap =>
...
export interface RoutingProviderProps {
     ...
     */
    onChange?: ((output: NavigationOutput) => void) | null;
     ...
     */
    basePath?: string;
    /** Called when an unhandled error occurs resolving a route. */
...
export interface RoutingProviderState {
    routes: RouteMap;
...
> {
    ...
    // Legacy React context API - `this.context` is intentionally untyped (`any`)
    // by @types/react for class components using the legacy context API.
    unlisten?: () => void;

    constructor(props: RoutingProviderProps) {
        super(props);
        invariant(
            props.routes &&
                (props.routes instanceof RouteMap || Array.isArray(props.routes)),
        ...
        if (props.location) {
            try {
                ...
                this.state = { ...this.state, validLocation };
            } catch (e) {
    ...
    componentWillReceiveProps(nextProps: RoutingProviderProps) {
    ...
    handleHistory = (location: { pathname: string; search: string }, action: string): void => {
        const path = location.pathname + location.search;
        this.doNavigation(path, action);
    };

    ensurePath(location: Location | string): string {
        const path =
    ...
    checkValidLocation(location: Location | null | undefined): void {
        try {
            this.setState({
                validLocation: !!this.ensurePath(location!)
            });
        } catch (error) {
            // Note: passing `error` (truthy) as the condition here matches the
            // pre-conversion JS behavior verbatim, which means this warning never
            // actually fires - preserved as-is rather than silently "fixed" as
            // part of a straight JS->TS port.
            warning(
                error as any,
    ...
    handleRedirect = (to: Location | string): void => {
    ...
    handleNavigate = (to: Location | string): void => {
        this.props.history.push(this.handleStringify(to));
    };

    doNavigation(fullPath: string, action: string): void {
        ...
        const series = branch.reduce((promise: Promise<Location> | null, leaf) => {
            if (!leaf.resolve) {
                return promise;
            }
            return (promise || Promise.resolve({})).then(reduced =>
                leaf.resolve!(location as Location, this.props.context!()).then(
                    result => {
    ...
    completeRouting = (output: NavigationOutput): void => {
    ...
    handleStringify = (location: Location | string): string => {
        const stringified = this.ensurePath(location);
        return joinPaths(this.props.basePath!, stringified);
    };

    handleGetLocation = (): Location | null | undefined => this.props.location;

    handleGetResolved = (): Location => this.props.resolved || DEFAULT_STATE;

    handleIsActive = (location: Location | string, exact = false): boolean => {
        ...
        const mappedRoute = (this.state.routes.routes as MappedRoute[]).find(
            route => route.route === currentBranch[toBranch.length - 1]
        );
        return (
            !!mappedRoute &&
            hydrateRoute(mappedRoute, this.props.location!) === toPath
        );
    };
